...
from qiskit.quantum_info import Statevector  # TODO: remove

from .variational import SingleParameterAnsatz
...
def circuit_overlap(U_var: QuantumCircuit, U_tilde: QuantumCircuit) -> QuantumCircuit:
    """
    Fig. S3(a): Hadamard-test for <ψ~|ψ>.
    Implements controlled U_var followed by controlled U_tilde^\dagger.
    """
    ...
    # U_tilde_dag_gate = U_tilde.to_gate(label="U_tilde_dag").inverse().control(1)
    # qc.append(U_tilde_dag_gate, [anc[0]] + list(sys))
...
def build_adder_block(anc, sys, qnpu, U_tilde: QuantumCircuit) -> QuantumCircuit:
    """
    Build the adder QNPU block.
    Returns a quantum circuit implementing the adder operation A.
    """
...
class CostFunction:
    ...
    def compute_cost_via_qc(self, lambdas):
        """
        ...
        #     "wDA": float(wDA),
        #     "LapVal": float(LapVal),
        #     "NonlinVal": float(NonlinVal),
        #     "s": float(s),
        #     "cost_up_to_const": float(cost_up_to_const),
        # }
