...
def build_adder_block(
    anc: QuantumRegister, 
    sys_x: QuantumRegister,
    qnpu: QuantumRegister,
    U_tilde: QuantumCircuit,
    sys_y: Optional[QuantumRegister]=None,
) -> QuantumCircuit:
    """
    ...
    """
    qc = QuantumCircuit(anc, qnpu, sys_x)
    qc.cx(control_qubit=anc[0], target_qubit=sys_x[0])
    qc.ccx(sys_x[0], anc[0], sys_x[1])
    qc.ccx(sys_x[0], anc[0], qnpu[0])
    qc.ccx(sys_x[1], qnpu[0], qnpu[1])
    qc.cx(control_qubit=qnpu[1], target_qubit=sys_x[2])
    qc.ccx(sys_x[2], qnpu[1], sys_x[3])
    qc.ccx(sys_x[1], qnpu[0], qnpu[1])
    qc.ccx(sys_x[0], anc[0], qnpu[0])
    ...
    U_tilde_dag_gate = U_tilde_gate.inverse()
    U_tilde_dag_gate.label = "U_tilde_dag"
    qc.append(U_tilde_dag_gate.control(1), [anc[0]] + list(sys_x))
